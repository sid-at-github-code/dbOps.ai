...
@router.post(
    ...
    description=(
        "Convert a plain-English question about the database into a validated "
    ...
    responses={
        400: {"model": V1ErrorResponse, "description": "Malformed JSON or missing required field."},
        401: {"model": V1ErrorResponse, "description": "API key absent, malformed, or revoked."},
        403: {"model": V1ErrorResponse, "description": "user_type exceeds the plan tier."},
        422: {"model": V1ErrorResponse, "description": "Could not produce valid SQL. Rephrase the query."},
        429: {
            "model": V1ErrorResponse,
            "description": "Rate limit exceeded.",
            "headers": {
                "X-RateLimit-Limit": {"description": "Max requests per window.", "schema": {"type": "integer"}},
                "X-RateLimit-Reset": {"description": "Window duration in seconds.", "schema": {"type": "integer"}},
            },
        },
        500: {"model": V1ErrorResponse, "description": "Unhandled server-side failure."},
    },
...
async def v1_run_query(
    ...
    try:
        ...
        if req.user_type is not None:
            allowed = _PLAN_ALLOWED_USER_TYPES.get(settings.v1_plan_tier, {"free", "basic"})
            if req.user_type not in allowed:
                return _error(
                    403,
                    "FORBIDDEN",
                    f"user_type '{req.user_type}' exceeds the plan tier bound to this API key.",
                )
        ...
        try:
            ...
            return _error(422, "CONVERSION_FAILED", "Could not produce valid SQL from the input.")

        try:
            validate_read_only(sql)
        except ValueError:
            log.warning("v1/query | non-SELECT SQL | sql=%r", sql)
            return _error(422, "CONVERSION_FAILED", "Could not produce valid SQL from the input.")
