"""
Unit tests for execute_query() and validate_read_only().
...
class TestValidateReadOnly:
    ...
    def test_with_cte_passes(self):
        validate_read_only(
            'WITH ranked AS (SELECT *, ROW_NUMBER() OVER (ORDER BY id) AS rn FROM t) '
    ...
    def test_execute_query_blocked_on_write(self):
        ...
        with patch("nl_sql.query_executor._get_connection") as mock_conn:
            with pytest.raises(ValueError):
...
class TestExecuteQuery:
    def test_returns_rows_and_elapsed(self):
        ...
        conn = _make_conn(cur)
        with patch("nl_sql.query_executor._get_connection", return_value=conn):
